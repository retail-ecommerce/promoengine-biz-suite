...
import styles from './ApplyCondition.preference.less'
...
const internalImageListOf = (applyCondition) =>{
...
const internalSettingListOf = (applyCondition) =>{
	const userContext = null
	const optionList = [ 
	]
...
const internalLargeTextOf = (applyCondition) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {ApplyConditionService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = ApplyConditionService[candidateServiceName] 
...
const executeTrans = (applyCondition,targetComponent) =>{
...
const buildTransferModal = (applyCondition,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(applyCondition,targetComponent)}
...
const internalRenderExtraHeader = (applyCondition) =>{
	return null
}
const internalRenderExtraFooter = (applyCondition) =>{
	return null
}
...
const internalSummaryOf = (applyCondition,targetComponent) =>{
    const userContext = null
	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="Id">{applyCondition.id}</Description> 
<Description term="Name">{applyCondition.name}</Description> 
<Description term="Last Update Time">{ moment(applyCondition.lastUpdateTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(applyCondition,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = applyCondition => {
  const {ApplyConditionBase} = GlobalComponents
  return <PermissionSetting targetObject={applyCondition}  targetObjectMeta={ApplyConditionBase}/>


}


class ApplyConditionPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, targetUserRuleCount } = this.props.applyCondition
    const cardsData = {cardsName:"Apply Condition",cardsFor: "applyCondition",cardsSource: this.props.applyCondition,
  		subItems: [
...
}))(Form.create()(ApplyConditionPreference))
