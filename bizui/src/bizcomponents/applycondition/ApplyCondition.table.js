...
import styles from './ApplyCondition.table.less'
...
class ApplyConditionTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ApplyConditionBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/applyCondition/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default ApplyConditionTable
