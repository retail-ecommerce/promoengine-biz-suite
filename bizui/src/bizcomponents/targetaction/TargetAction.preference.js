...
import styles from './TargetAction.preference.less'
...
const internalImageListOf = (targetAction) =>{
...
const internalSettingListOf = (targetAction) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TargetActionService[candidateServiceName] 
...
const executeTrans = (targetAction,targetComponent) =>{
...
const buildTransferModal = (targetAction,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(targetAction,targetComponent)}
...
const internalRenderExtraHeader = (targetAction) =>{
	return null
}
const internalRenderExtraFooter = (targetAction) =>{
	return null
}
...
<Description term="Action">{targetAction.action==null?appLocaleName(userContext,"NotAssigned"):targetAction.action.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Action","actionType","requestCandidateAction",
	      "transferToAnotherAction","anotherActionId",targetAction.action?targetAction.action.id:"")} 
...
<Description term="Promotion">{targetAction.promotion==null?appLocaleName(userContext,"NotAssigned"):targetAction.promotion.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Promotion","promotion","requestCandidatePromotion",
	      "transferToAnotherPromotion","anotherPromotionId",targetAction.promotion?targetAction.promotion.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = targetAction => {
  const {TargetActionBase} = GlobalComponents
  return <PermissionSetting targetObject={targetAction}  targetObjectMeta={TargetActionBase}/>


}


class TargetActionPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Target Action",cardsFor: "targetAction",cardsSource: this.props.targetAction,
  		subItems: [
...
}))(Form.create()(TargetActionPreference))
