...
const showAssociateDialog = (targetComponent) => {
  ...
  const { ActionTypeAssociateForm } = GlobalComponents
  ...
  <div>
    ...
    <ActionTypeAssociateForm 
	visible={currentAssociateModal==='action'} 
	data={{targetActionList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'action')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'action')}/> <PromotionAssociateForm 
...
class TargetActionSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {TargetActionTable} = GlobalComponents;
    const {TargetActionSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TargetActionSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,TargetActionModalTable,"targetActionIds")}
...
export default Form.create()(TargetActionSearch)
