...
import styles from './ActionType.preference.less'
...
const internalImageListOf = (actionType) =>{
...
const internalSettingListOf = (actionType) =>{
	const userContext = null
	const optionList = [ 
	]
...
const internalLargeTextOf = (actionType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {ActionTypeService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = ActionTypeService[candidateServiceName] 
...
const executeTrans = (actionType,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_actionType/doJob",payload: payload})
...
const buildTransferModal = (actionType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(actionType,targetComponent)}
...
const internalRenderExtraHeader = (actionType) =>{
	return null
}
const internalRenderExtraFooter = (actionType) =>{
	return null
}
...
const internalSummaryOf = (actionType,targetComponent) =>{
    const userContext = null
	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="Id">{actionType.id}</Description> 
<Description term="Name">{actionType.name}</Description> 
<Description term="Last Update Time">{ moment(actionType.lastUpdateTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(actionType,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = actionType => {
  const {ActionTypeBase} = GlobalComponents
  return <PermissionSetting targetObject={actionType}  targetObjectMeta={ActionTypeBase}/>


}


class ActionTypePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, targetActionCount } = this.props.actionType
    const cardsData = {cardsName:"Action Type",cardsFor: "actionType",cardsSource: this.props.actionType,
  		subItems: [
...
}))(Form.create()(ActionTypePreference))
