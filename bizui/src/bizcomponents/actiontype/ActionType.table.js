...
import styles from './ActionType.table.less'
...
class ActionTypeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ActionTypeBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/actionType/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
