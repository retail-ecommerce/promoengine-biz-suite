...
const showAssociateDialog = (targetComponent) => {
  ...
  const { PromotionLevelAssociateForm } = GlobalComponents
  const { PromotionAssociateForm } = GlobalComponents
  ...
  <div>
    ...
    <PromotionLevelAssociateForm 
	visible={currentAssociateModal==='promotionLevel'} 
	data={{promotionOfferList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'promotionLevel')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'promotionLevel')}/> <PromotionAssociateForm 
	visible={currentAssociateModal==='promotion'} 
	data={{promotionOfferList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'promotion')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'promotion')}/> 
...
class PromotionOfferSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {PromotionOfferTable} = GlobalComponents;
    const {PromotionOfferSearchForm} = GlobalComponents;
    const {PromotionOfferModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PromotionOfferSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,PromotionOfferModalTable,"promotionOfferIds")}
...
export default Form.create()(PromotionOfferSearch)
