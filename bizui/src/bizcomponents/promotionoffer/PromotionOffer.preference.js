...
import styles from './PromotionOffer.preference.less'
...
const internalImageListOf = (promotionOffer) =>{
...
const internalSettingListOf = (promotionOffer) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PromotionOfferService[candidateServiceName] 
...
const executeTrans = (promotionOffer,targetComponent) =>{
...
const buildTransferModal = (promotionOffer,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(promotionOffer,targetComponent)}
...
const internalRenderExtraHeader = (promotionOffer) =>{
	return null
}
const internalRenderExtraFooter = (promotionOffer) =>{
	return null
}
...
<Description term="Promotion Level">{promotionOffer.promotionLevel==null?appLocaleName(userContext,"NotAssigned"):promotionOffer.promotionLevel.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Promotion Level","promotionLevel","requestCandidatePromotionLevel",
	      "transferToAnotherPromotionLevel","anotherPromotionLevelId",promotionOffer.promotionLevel?promotionOffer.promotionLevel.id:"")} 
...
<Description term="Promotion">{promotionOffer.promotion==null?appLocaleName(userContext,"NotAssigned"):promotionOffer.promotion.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Promotion","promotion","requestCandidatePromotion",
	      "transferToAnotherPromotion","anotherPromotionId",promotionOffer.promotion?promotionOffer.promotion.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = promotionOffer => {
  const {PromotionOfferBase} = GlobalComponents
  return <PermissionSetting targetObject={promotionOffer}  targetObjectMeta={PromotionOfferBase}/>


}


class PromotionOfferPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Promotion Offer",cardsFor: "promotionOffer",cardsSource: this.props.promotionOffer,
  		subItems: [
...
}))(Form.create()(PromotionOfferPreference))
