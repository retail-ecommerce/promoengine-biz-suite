...
import styles from './Promotion.updateform.less'
...
class PromotionUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        validAfter: moment(item.validAfter),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, promotionId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updatePromotion`,
          payload: {
            id: owner.id,
            type: 'promotion',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextPromotionUpdateRow`,
        payload: {
          id: owner.id,
          type: 'promotion',
          selectedRows,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.validAfter} {...formItemLayout}>
                  {getFieldDecorator('validAfter', {
                    initialValue: selectedRow.validAfter,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(PromotionUpdateForm))
