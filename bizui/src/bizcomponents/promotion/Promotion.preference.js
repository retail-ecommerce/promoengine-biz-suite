...
import styles from './Promotion.preference.less'
...
const internalImageListOf = (promotion) =>{
...
const internalSettingListOf = (promotion) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PromotionService[candidateServiceName] 
...
const executeTrans = (promotion,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_promotion/doJob",payload: payload})
...
const buildTransferModal = (promotion,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(promotion,targetComponent)}
...
const internalRenderExtraHeader = (promotion) =>{
	return null
}
const internalRenderExtraFooter = (promotion) =>{
	return null
}
...
const renderPermissionSetting = promotion => {
  const {PromotionBase} = GlobalComponents
  return <PermissionSetting targetObject={promotion}  targetObjectMeta={PromotionBase}/>


}


class PromotionPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, targetUserRuleCount, targetActionCount, promotionOfferCount } = this.props.promotion
    const cardsData = {cardsName:"Promotion",cardsFor: "promotion",cardsSource: this.props.promotion,
  		subItems: [
...
}))(Form.create()(PromotionPreference))
