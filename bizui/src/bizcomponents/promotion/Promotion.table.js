...
import styles from './Promotion.table.less'
...
class PromotionTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = PromotionBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/promotion/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
