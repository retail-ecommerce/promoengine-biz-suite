...
import styles from './PromotionLevel.table.less'
...
class PromotionLevelTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = PromotionLevelBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/promotionLevel/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default PromotionLevelTable
