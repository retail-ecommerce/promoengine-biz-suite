...
import styles from './PromotionLevel.preference.less'
...
const internalImageListOf = (promotionLevel) =>{
...
const internalSettingListOf = (promotionLevel) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PromotionLevelService[candidateServiceName] 
...
const executeTrans = (promotionLevel,targetComponent) =>{
...
const buildTransferModal = (promotionLevel,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(promotionLevel,targetComponent)}
...
const internalRenderExtraHeader = (promotionLevel) =>{
	return null
}
const internalRenderExtraFooter = (promotionLevel) =>{
	return null
}
...
const renderPermissionSetting = promotionLevel => {
  const {PromotionLevelBase} = GlobalComponents
  return <PermissionSetting targetObject={promotionLevel}  targetObjectMeta={PromotionLevelBase}/>


}


class PromotionLevelPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, promotionOfferCount } = this.props.promotionLevel
    const cardsData = {cardsName:"Promotion Level",cardsFor: "promotionLevel",cardsSource: this.props.promotionLevel,
  		subItems: [
...
}))(Form.create()(PromotionLevelPreference))
