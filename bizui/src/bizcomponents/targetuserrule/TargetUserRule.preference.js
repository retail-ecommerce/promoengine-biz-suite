...
import styles from './TargetUserRule.preference.less'
...
const internalImageListOf = (targetUserRule) =>{
...
const internalSettingListOf = (targetUserRule) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TargetUserRuleService[candidateServiceName] 
...
const executeTrans = (targetUserRule,targetComponent) =>{
...
const buildTransferModal = (targetUserRule,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(targetUserRule,targetComponent)}
...
const internalRenderExtraHeader = (targetUserRule) =>{
	return null
}
const internalRenderExtraFooter = (targetUserRule) =>{
	return null
}
...
<Description term="Apply Condition">{targetUserRule.applyCondition==null?appLocaleName(userContext,"NotAssigned"):targetUserRule.applyCondition.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Apply Condition","applyCondition","requestCandidateApplyCondition",
	      "transferToAnotherApplyCondition","anotherApplyConditionId",targetUserRule.applyCondition?targetUserRule.applyCondition.id:"")} 
...
<Description term="Promotion">{targetUserRule.promotion==null?appLocaleName(userContext,"NotAssigned"):targetUserRule.promotion.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Promotion","promotion","requestCandidatePromotion",
	      "transferToAnotherPromotion","anotherPromotionId",targetUserRule.promotion?targetUserRule.promotion.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = targetUserRule => {
  const {TargetUserRuleBase} = GlobalComponents
  return <PermissionSetting targetObject={targetUserRule}  targetObjectMeta={TargetUserRuleBase}/>


}


class TargetUserRulePreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Target User Rule",cardsFor: "targetUserRule",cardsSource: this.props.targetUserRule,
  		subItems: [
...
}))(Form.create()(TargetUserRulePreference))
