...
const showAssociateDialog = (targetComponent) => {
  ...
  const { ApplyConditionAssociateForm } = GlobalComponents
  ...
  <div>
    ...
    <ApplyConditionAssociateForm 
	visible={currentAssociateModal==='applyCondition'} 
	data={{targetUserRuleList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'applyCondition')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'applyCondition')}/> <PromotionAssociateForm 
...
class TargetUserRuleSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {TargetUserRuleTable} = GlobalComponents;
    const {TargetUserRuleSearchForm} = GlobalComponents;
    const {TargetUserRuleModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TargetUserRuleSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,TargetUserRuleModalTable,"targetUserRuleIds")}
...
export default Form.create()(TargetUserRuleSearch)
